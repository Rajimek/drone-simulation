...
SimulationModel::SimulationModel(IController& controller)
  ...
  entityFactory.AddFactory(new ChargingPadFactory());

  // set up notifications
  notificationSubject = new NotificationSubject();
  notificationObserver = new NotificationObserver(notificationSubject);

  // set up the recharge observer
  rechargeSubject = new RechargeSubject();
  rechargeObserver = new RechargeObserver(rechargeSubject);
}
...
IEntity* SimulationModel::createEntity(JsonObject& entity) {
  ...
  if (myNewEntity = entityFactory.CreateEntity(entity)) {
    ...
    // check if it is a drone and apply the BatteryDecorator
    if (Drone* drone = dynamic_cast<Drone*>(myNewEntity)) {
      myNewEntity =
          new BatteryDecorator(const_cast<JsonObject&>(drone->getDetails()));
    }

    if (ChargingPad* chargingPad = dynamic_cast<ChargingPad*>(myNewEntity)) {
      this->chargingPad = chargingPad;
    }
...
void SimulationModel::update(double dt) {
  ...
  // check the notificationObserver for any updates
  JsonObject message = notificationObserver->GetMessageFromSubject();
  if (notificationObserver->HasNewMessage()) {
    // send data to the frontend
    controller.sendEventToView("Notification", message);
  }
}
